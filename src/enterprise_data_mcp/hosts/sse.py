"""Legacy SSE transport host — /sse · /messages; loopback only (Step 12)."""
...
def main() -> None:
    settings = load_sse_settings(os.environ)
    container = resolve_app_container(settings, os.environ)
    app = build_sse_app(container=container, settings=settings)
    uvicorn.run(
        app,
        host=settings.host,
        port=settings.port,
